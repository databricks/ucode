...
The proxy address is different for each launch, so Codex's provider must remain in the ucode profile
and launch-time overrides. During configuration, current versions remove provider values that older
ucode versions wrote to `/etc/codex/managed_config.toml`; Codex gives that file higher precedence
and would otherwise silently bypass the limiter.
...
Claude Code configuration installs gateway-critical values in the OS-managed settings scope so
enterprise settings cannot silently override Unity Gateway. Codex keeps its provider launch-scoped
because its shared limiter uses a per-session loopback proxy. Codex configuration retires provider
values written there by older ucode versions and stops if unrelated machine policy still overrides
the launch-scoped provider. Non-interactive and CI runs never invoke `sudo`. Claude subscription
relay is also local-only because its loopback proxy exists only for that session.
...
| `/etc/codex/managed_config.toml` | Codex OS-managed settings inspected for provider conflicts; older ucode-owned values are retired |
