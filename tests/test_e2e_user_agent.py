...
class TestOpencodeUserAgent:
    def test_user_agent_arrives_at_gateway(self, tmp_path, monkeypatch, capture_server):
        ...
        with pytest.MonkeyPatch().context() as mp:
            mp.setattr("ucode.state.save_state", lambda s: None)
            mp.setattr(
                "ucode.agents.opencode.get_databricks_token", lambda ws, profile=None, **kwargs: "test-token"
            )
...
class TestPiUserAgent:
    def test_user_agent_arrives_at_gateway(self, tmp_path, monkeypatch, capture_server):
        ...
        with pytest.MonkeyPatch().context() as mp:
            mp.setattr("ucode.state.save_state", lambda s: None)
            mp.setattr("ucode.agents.pi.get_databricks_token", lambda ws, profile=None, **kwargs: "test-token")
